...
#' created by a called to \code{dbSendStatement} as the JDBC API does not return
#' a ResultSet but rather an integer value. The \code{dbGetRowsAffected} called
#' on \code{SQLServerUpdateResult} returns the value produced by the JDBC API.
#' The \code{SQLServerPreResult} class wraps a JDBC PreparedStatement that has
#' yet to be executed and is necessary for \code{dbBind} to function per the DBI
#' spec (taking a DBIResult as an input and returning another as output). This
#' contrasts to the JDBC interface where binding occurs on Statement classes and
#' not on ResultSet classes.
#'
#' @keywords internal
